/**
 * Persisted corpus on Vercel Blob. Secrets (BLOB_READ_WRITE_TOKEN) live only
 * in the Vercel project — never in the public repo.
 */
...
export async function readCorpusFromBlob(): Promise<Corpus | null> {
  ...
  try {
    ...
    const match = blobs.find((b) => b.pathname === CORPUS_BLOB_PATHNAME)
      ?? blobs[0];
